"""
Step 9 follow-up #8: gradient-boosted trees vs ridge, validated on the same
held-out nested-by-signing-year harness as step 8.

Hypothesis: a tree ensemble can find age x position x RAR interactions
automatically that hand-built interaction terms (age_x_krate, rar_x_hra)
miss, without needing the analyst to guess the right interaction. High
overfitting risk at n~210 (84 batters, 130 pitchers after the floor) is the
documented concern in step9_followup_plan.md -- this script's whole point
is to check empirically whether that risk materializes or whether GBM
actually generalizes better than ridge here.

Uses sklearn.ensemble.HistGradientBoostingRegressor (no extra install
needed) with light regularization (early stopping via validation fraction,
shallow trees, L2) and an optional monotonic constraint on proj_rar (more
production should never predict lower pay, holding everything else fixed)
since lightgbm is not confirmed installed in this env.

Outputs: prints comparison to ridge baseline; saves
intermediate/step9e_gbm_results.csv
"""
...
print("=" * 60)
print("STEP 9 FOLLOW-UP #8: GBM vs ridge (held-out validated)")
print("=" * 60)
...
# GBM hyperparams chosen conservatively for n~150-180 per fold per player type:
# shallow trees, few leaves, strong L2, small learning rate with early-stopping
# validation split. monotonic_cst: +1 on proj_rar / proj_rar_sq / bat_raa /
# pit_raa (more production should never reduce predicted pay, all else equal).
GBM_PARAMS = dict(
    max_depth=3, max_leaf_nodes=8, learning_rate=0.05,
    max_iter=300, l2_regularization=1.0,
    validation_fraction=0.2, n_iter_no_change=15,
    random_state=42,
)

def fit_gbm(train_df, feature_cols, monotonic_map):
    """Fit a HistGradientBoostingRegressor on log(AAV) with per-feature monotonic constraints from monotonic_map."""
    X = train_df[feature_cols].values.astype(float)
    y = train_df["log_aav"].values.astype(float)
    mono = [monotonic_map.get(f, 0) for f in feature_cols]
    model = HistGradientBoostingRegressor(monotonic_cst=mono, **GBM_PARAMS)
    model.fit(X, y)
    return model

BAT_MONO = {"bat_raa": 1, "proj_ubr": 1, "proj_def": 1, "proj_rar_sq": 1}
PIT_MONO = {"proj_rar": 1, "proj_rar_sq": 1, "k_rate": 1}

print("\n[2] Nested-by-signing-year validation: ridge vs GBM...")

methods = {"ridge": "ridge", "gbm": "gbm"}
all_rows = []
for method in methods:
    ...
    for test_year in FOLD_YEARS:
        ...
        if len(test_m) == 0:
            continue
        fold_preds = []
        for ptype, feats, mono in [("batter", BAT_FEATURES, BAT_MONO), ("pitcher", PIT_FEATURES, PIT_MONO)]:
    ...
    r2 = 1 - ss_res / ss_tot
    print(f"  {method:8s} n={len(fold_all):3d}  within±15%={within15:.1%}  "
          f"median_err={med_err:.1%}  R^2={r2:.3f}")

results_all = pd.concat(all_rows, ignore_index=True)
results_all.to_csv(os.path.join(INT, "step9e_gbm_results.csv"), index=False)

print("\n" + "=" * 60)
print("STEP 9 FOLLOW-UP #8 COMPLETE")
print("  Outputs: intermediate/step9e_gbm_results.csv")
